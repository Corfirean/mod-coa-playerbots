...
- **Would a bot take it?** (`QuestKnowledge::supported`) — acceptance policy plus what the data
  ...
  item-started, elite (unless `AcceptEliteQuests`).
...
think a quest is doable while another can never create a task for it. In this phase only kill
objectives have a handler, so bots take **kill quests and plain delivery quests** (no objectives,
just a turn-in); collect quests arrive in Phase 4 with the reverse loot index they need.
