...
- `.botcmd brain <guid>` — goal (and suspension reason), task, phase and time in phase (PAUSED and
  ...
  detour/session/break state, temporary party, next planner pass, last event, per-bot counters
...
progress, quest policy, social rules) is unit-tested standalone (`module/tests/`, also under
